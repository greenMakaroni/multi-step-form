...
const Sidebar = ({ step, setStep, isDesktop }) => {
    ...
    return (
        <div className={`flex ${isDesktop ? " bg-[length:100%_100%] bg-sidebar_desktop w-[30%] rounded-xl flex-col px-[25px] py-[25px]" : "bg-cover w-screen h-1/5 bg-sidebar_mobile flex-row items-center justify-center"}`}>
            {steps.map((s, index) => {
                return (
                    <div key={index} className={`flex flex-row m-[0] my-[10px] ${!isDesktop && "mx-[5px] mt-[-30px]"}`}>
                        ...
                        {isDesktop &&
                            <div className="ml-[10px] flex flex-col justify-center">
                                ...
                                <p className="font-bold text-[16px] tracking-widest font-['Ubuntu'] font-[500]">{s.text}</p>
                            </div>}
                    </div>
                )
            })}
